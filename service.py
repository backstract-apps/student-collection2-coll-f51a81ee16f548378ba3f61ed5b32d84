...
async def get_student(db: Session, user_id: str, name: str, email: str, password: str, role: str):
...
async def post_student(db: Session, user_id: int, name: str, email: str, password: str, role: str):
    ...
    id = db.query(models.Student).filter(models.Student.user_id == user_id).first()
...
async def put_tasks_id(db: Session, id: int, title: str, description: str, status: str, priority: str):
